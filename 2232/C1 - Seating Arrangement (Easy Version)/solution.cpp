...
 
void solve() {
    int n, tables, seatsPerTable;
    cin >> n >> tables >> seatsPerTable;
 
    string line;
    cin >> line;
 
    int totalAmbiverts = count(line.begin(), line.end(), 'A');
 
    int answer = 0;
 
    for (int ambivertsOpening = 0; ambivertsOpening <= totalAmbiverts; ambivertsOpening++) {
 
        int emptyTables = tables;
        int freeSeats = 0;
        int seatedPeople = 0;
 
        int remainingAmbivertsOpening = ambivertsOpening;
 
...
 
                if (remainingAmbivertsOpening > 0 && emptyTables > 0) {
 
                    emptyTables--;
                    freeSeats += seatsPerTable - 1;
                    seatedPeople++;
 
                    remainingAmbivertsOpening--;
                }
                else if (freeSeats > 0) {
 
                    freeSeats--;
...
 
        answer = max(answer, seatedPeople);
    }
 
    cout << answer << '
';
